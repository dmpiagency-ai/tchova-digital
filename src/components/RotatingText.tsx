...
type AnimationInitial = boolean | AnimationTarget;
      ...
      <AnimatePresence mode={animatePresenceMode} initial={animatePresenceInitial}>
        ...
        >
          {elements.map((wordObj: WordObj, wordIndex: number, array: WordObj[]) => {
            ...
            const totalChars = array.reduce((sum: number, word: WordObj) => sum + word.characters.length, 0);
            return (
              <span key={wordIndex} className={cn('text-rotate-word', splitLevelClassName)}>
                {wordObj.characters.map((char: string, charIndex: number) => (
                  <motion.span
                    key={charIndex}
                    initial={initial as AnimationInitial}
                    animate={animate as AnimationTarget}
                    exit={exit as AnimationTarget}
                    transition={{
                      ...transition,
